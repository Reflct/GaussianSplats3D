...
const compressionLevel =
  process.argv.length >= 5 ? parseInt(process.argv[4]) : undefined;
const splatAlphaRemovalThreshold =
  process.argv.length >= 6 ? parseInt(process.argv[5]) : undefined;
const sceneCenter =
  process.argv.length >= 7
    ? new THREE.Vector3().fromArray(process.argv[6].split(","))
    : undefined;
const blockSize =
  process.argv.length >= 8 ? parseFloat(process.argv[7]) : undefined;
const bucketSize =
  process.argv.length >= 9 ? parseInt(process.argv[8]) : undefined;
const outSphericalHarmonicsDegree =
  process.argv.length >= 10 ? parseInt(process.argv[9]) : undefined;
const sectionSize = 0;
