...
/**
 * Interface for intersection hit results
 */
...
export class Ray {
   ...
   * Set ray parameters
   */
  setParameters(origin: THREE.Vector3, direction: THREE.Vector3): void {
  ...
  intersectBox = (() => {
    ...
    const planeIntersectionPointArray: number[] = [];
    const originArray: number[] = [];
    const directionArray: number[] = [];

    return (box: THREE.Box3, outHit?: RayHit): boolean => {
