...
// Define interface for material uniforms
...
export class SceneHelper {
  ...
  splatRenderTarget: THREE.WebGLRenderTarget | null;
  renderTargetCopyQuad: THREE.Mesh<
    THREE.PlaneGeometry,
    THREE.ShaderMaterial
  > | null;
  renderTargetCopyCamera: THREE.OrthographicCamera | null;
  meshCursor: THREE.Object3D | null;
  focusMarker: THREE.Mesh<THREE.SphereGeometry, THREE.ShaderMaterial> | null;
  controlPlane: THREE.Object3D | null;
  debugRoot: THREE.Object3D | null;
  secondaryDebugRoot: THREE.Object3D | null;
  ...
  updateSplatRenderTargetForRenderDimensions(
    width: number,
    height: number
  ): void {
  ...
  setupRenderTargetCopyObjects(): void {
    ...
    // Enable fragment depth extension through any casting
    (renderTargetCopyMaterial as any).extensions.fragDepth = true;
  ...
  destroyRenderTargetCopyObjects(): void {
    if (this.renderTargetCopyQuad) {
      disposeAllMeshes(this.renderTargetCopyQuad as any);
      this.renderTargetCopyQuad = null;
    }
  }
  ...
  destroyMeshCursor(): void {
    if (this.meshCursor) {
      disposeAllMeshes(this.meshCursor as any);
  ...
  getMeschCursorVisibility(): boolean {
    return this.meshCursor ? this.meshCursor.visible : false;
  }

  setMeshCursorPosition(position: THREE.Vector3): void {
    if (this.meshCursor) {
      this.meshCursor.position.copy(position);
    }
  }

  positionAndOrientMeshCursor(
  ...
  destroyFocusMarker(): void {
    if (this.focusMarker) {
      disposeAllMeshes(this.focusMarker as any);
      this.focusMarker = null;
    }
  }
  ...
  getFocusMarkerOpacity(): number {
    return this.focusMarker
      ? this.focusMarker.material.uniforms.opacity.value
      : 0;
  }
  ...
  destroyControlPlane(): void {
    if (this.controlPlane) {
      disposeAllMeshes(this.controlPlane as any);
      this.controlPlane = null;
    }
  }
  ...
  createDebugMeshes(renderOrder?: number): THREE.Object3D {
    const sphereGeometry = new THREE.SphereGeometry(1, 32, 32);
    const debugMeshRoot = new THREE.Object3D();

    const createMesh = (color: number, position: [number, number, number]) => {
  ...
  createSecondaryDebugMeshes(renderOrder?: number): THREE.Object3D {
    const boxGeometry = new THREE.BoxGeometry(3, 3, 3);
    const debugMeshRoot = new THREE.Object3D();

    let boxColor = 0xbbbbbb;
    const createMesh = (position: [number, number, number]) => {
  ...
  static buildDebugMaterial(color: number): THREE.ShaderMaterial {
    ...
    // Enable fragment depth extension through any casting
    (material as any).extensions.fragDepth = true;

    return material;
  }

  static buildFocusMarkerMaterial(color?: number): THREE.ShaderMaterial {
    const vertexShaderSource = `
            #include <common>
