...
export class PlayCanvasCompressedPlyEncoder {
  ...
  static revertElementStorageArrays(storageArrays) {
    ...
    for (const prop of chunkProperties) {
      ...
      prop.storageSizeByes = prop.byteSize * chunkElement.count;
      chunkElement.properties.push(prop);
      chunkElement.storageSizeBytes += prop.storageSizeByes;
    }
    ...
    for (const prop of vertexProperties) {
      ...
      prop.storageSizeByes = prop.byteSize * vertexElement.count;
      vertexElement.properties.push(prop);
      vertexElement.storageSizeBytes += prop.storageSizeByes;
    }
      ...
      for (let i = 0; i < 45; i++) {
        ...
        if (storage) {
          const prop = {
            ...
            storageSizeByes: prop.byteSize * shElement.count,
          };
  ...
  static readSplatDataFromSectionBuffer = (function () {
    const tempCenter = new THREE.Vector3();
    const tempScale = new THREE.Vector3();
    const tempRotation = new THREE.Quaternion();
    const tempColor = new Uint8Array(4);
    const tempSH = new Float32Array(64);

    const decompressPositionOffset = (
      v,
      compressionScaleFactor,
      compressionScaleRange
    ) => {
      return (v - compressionScaleRange) / compressionScaleFactor;
    };
    ...
    return function (
      ...
      const colorView = new Uint8Array(
        buffer,
        offset +
          SplatBuffer.CompressionLevels[compressionLevel].ColorOffsetBytes,
        4
      );

      // Read and decompress position
      const x = dataViewFloatForCompressionLevel(dataView, 0, compressionLevel);
      const y = dataViewFloatForCompressionLevel(dataView, 1, compressionLevel);
      const z = dataViewFloatForCompressionLevel(dataView, 2, compressionLevel);

      if (compressionLevel >= 1) {
        tempCenter.x =
          decompressPositionOffset(
            x,
            compressionScaleFactor,
            compressionScaleRange
          ) + bucketCenter.x;
      ...
      // Read and decompress scale
      const scaleOffset =
        SplatBuffer.CompressionLevels[compressionLevel].ScaleOffsetBytes;
      const scaleDataView = new DataView(buffer, offset + scaleOffset);
      tempScale.set(
        toUncompressedFloat(
          dataViewFloatForCompressionLevel(scaleDataView, 0, compressionLevel),
          compressionLevel
        ),
        toUncompressedFloat(
          dataViewFloatForCompressionLevel(scaleDataView, 1, compressionLevel),
          compressionLevel
        ),
        toUncompressedFloat(
          dataViewFloatForCompressionLevel(scaleDataView, 2, compressionLevel),
          compressionLevel
        )
      );

      // Read and decompress rotation
      const rotationOffset =
        SplatBuffer.CompressionLevels[compressionLevel].RotationOffsetBytes;
      const rotationDataView = new DataView(buffer, offset + rotationOffset);
      tempRotation.set(
        toUncompressedFloat(
      ...
      // Read color
      tempColor[0] = colorView[0];
      tempColor[1] = colorView[1];
      tempColor[2] = colorView[2];
      tempColor[3] = colorView[3];
      ...
      if (shDegree > 0) {
        ...
        const shDataView = new DataView(buffer, offset + shOffset);
        const shCoeffCount = (shDegree + 1) * (shDegree + 1);

        for (let i = 0; i < shCoeffCount; i++) {
          tempSH[i] = toUncompressedFloat(
            dataViewFloatForCompressionLevel(shDataView, i, compressionLevel),
            compressionLevel,
            minSphericalHarmonicsCoeff,
            maxSphericalHarmonicsCoeff
          );
      ...
      return {
        ...
        color: new Uint8Array(tempColor),
        sphericalHarmonics: shDegree > 0 ? new Float32Array(tempSH) : null,
      };
    };
  })();
}
