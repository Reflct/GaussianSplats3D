...
exports.INRIAV2PlyParser = void 0;
...
const [CB_FEATURES_DC, CB_FEATURES_REST_0, CB_FEATURES_REST_3, CB_OPACITY, CB_SCALING, CB_ROTATION_RE, CB_ROTATION_IM,] = [0, 1, 4, 16, 17, 18, 19];
...
const [PLY_SCALE_0, PLY_SCALE_1, PLY_SCALE_2, PLY_ROT_0, PLY_ROT_1, PLY_ROT_2, PLY_ROT_3, PLY_X, PLY_Y, PLY_Z, PLY_F_DC_0, PLY_F_DC_1, PLY_F_DC_2, PLY_OPACITY,] = FieldsToReadIndexes;
...
class INRIAV2PlyParser {
    ...
    static decodeSectionHeadersFromHeaderLines(headerLines) {
        ...
        const sectionNames = PlyParserUtils_1.PlyParserUtils.getHeaderSectionNames(headerLines);
        ...
        while (!lastSectionFound) {
            ...
            if (sectionIndex === codeBookSectionIndex) {
                sectionHeader = PlyParserUtils_1.PlyParserUtils.decodeSectionHeader(headerLines, codeBookEntriesToReadIdMap, currentStartLine);
            }
            else {
                sectionHeader = PlyParserUtils_1.PlyParserUtils.decodeSectionHeader(headerLines, fieldNameIdMap, currentStartLine);
            }
    ...
    static decodeSectionHeadersFromHeaderText(headerText) {
        const headerLines = PlyParserUtils_1.PlyParserUtils.convertHeaderTextToLines(headerText);
    ...
    static decodeHeaderFromHeaderText(headerText) {
        const headerSizeBytes = headerText.indexOf(PlyParserUtils_1.PlyParserUtils.HeaderEndToken) +
            PlyParserUtils_1.PlyParserUtils.HeaderEndToken.length +
            1;
    ...
    static decodeHeaderFromBuffer(plyBuffer) {
        const headerText = PlyParserUtils_1.PlyParserUtils.readHeaderFromBuffer(plyBuffer);
    ...
    static decodeCodeBook(codeBookData, sectionHeader) {
        ...
        for (let row = 0; row < sectionHeader.vertexCount; row++) {
            PlyParserUtils_1.PlyParserUtils.readVertex(codeBookData, sectionHeader, row, 0, CodeBookEntriesToReadIndexes, rawVertex, false);
    ...
    static readSplat(splatData, header, row, dataOffset, rawSplat) {
        return PlyParserUtils_1.PlyParserUtils.readVertex(splatData, header, row, dataOffset, FieldsToReadIndexes, rawSplat, false);
    }
    /**
    ...
    static parseToUncompressedSplatArray(plyBuffer, outSphericalHarmonicsDegree = 0) {
        ...
        const unified = new UncompressedSplatArray_1.UncompressedSplatArray(outSphericalHarmonicsDegree);
...
exports.INRIAV2PlyParser = INRIAV2PlyParser;
/**
 * Parses a splat to an uncompressed format
 */
INRIAV2PlyParser.parseToUncompressedSplat = (function () {
    let rawSplat = [];
    ...
    const OFFSET_SCALE0 = UncompressedSplatArray_1.UncompressedSplatArray.OFFSET.SCALE0;
    const OFFSET_SCALE1 = UncompressedSplatArray_1.UncompressedSplatArray.OFFSET.SCALE1;
    const OFFSET_SCALE2 = UncompressedSplatArray_1.UncompressedSplatArray.OFFSET.SCALE2;
    const OFFSET_ROTATION0 = UncompressedSplatArray_1.UncompressedSplatArray.OFFSET.ROTATION0;
    const OFFSET_ROTATION1 = UncompressedSplatArray_1.UncompressedSplatArray.OFFSET.ROTATION1;
    const OFFSET_ROTATION2 = UncompressedSplatArray_1.UncompressedSplatArray.OFFSET.ROTATION2;
    const OFFSET_ROTATION3 = UncompressedSplatArray_1.UncompressedSplatArray.OFFSET.ROTATION3;
    const OFFSET_FDC0 = UncompressedSplatArray_1.UncompressedSplatArray.OFFSET.FDC0;
    const OFFSET_FDC1 = UncompressedSplatArray_1.UncompressedSplatArray.OFFSET.FDC1;
    const OFFSET_FDC2 = UncompressedSplatArray_1.UncompressedSplatArray.OFFSET.FDC2;
    const OFFSET_OPACITY = UncompressedSplatArray_1.UncompressedSplatArray.OFFSET.OPACITY;
    const OFFSET_FRC = [];
    for (let i = 0; i < 45; i++) {
        OFFSET_FRC[i] = UncompressedSplatArray_1.UncompressedSplatArray.OFFSET.FRC0 + i;
    }
    return function (splatData, row, header, codeBook, splatDataOffset = 0, outSphericalHarmonicsDegree = 0) {
        ...
        newSplat[OFFSET_FDC0] = (0, Util_1.clamp)(Math.floor(newSplat[OFFSET_FDC0]), 0, 255);
        newSplat[OFFSET_FDC1] = (0, Util_1.clamp)(Math.floor(newSplat[OFFSET_FDC1]), 0, 255);
        newSplat[OFFSET_FDC2] = (0, Util_1.clamp)(Math.floor(newSplat[OFFSET_FDC2]), 0, 255);
        newSplat[OFFSET_OPACITY] = (0, Util_1.clamp)(Math.floor(newSplat[OFFSET_OPACITY]), 0, 255);
