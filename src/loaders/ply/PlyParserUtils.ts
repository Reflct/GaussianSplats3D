 ...
 */
const FieldSizeStringMap: { [key: string]: FieldSizeId } = {
 ...
 */
const FieldSize: { [key in FieldSizeId]: number } = {
...
/**
 * Interface for spherical harmonics info
 */
interface SphericalHarmonicsInfo {
  degree: number;
  coefficientsPerChannel: number;
  degree1Fields: number[];
  degree2Fields: number[];
}
...
export class PlyParserUtils {
   ...
   */
  static decodeSectionHeader(
    headerLines: string[],
    fieldNameIdMap: { [key: string]: number },
    headerStartLine: number = 0
  ): SectionHeader {
    ...
    const fieldTypes: FieldSizeId[] = [];
    const allFieldNames: string[] = [];
    const usedFieldNames: string[] = [];
    const fieldTypesByName: { [key: string]: string } = {};
    ...
    const sphericalHarmonics =
      PlyParserUtils.decodeSphericalHarmonicsFromSectionHeader(
        allFieldNames,
        fieldNameIdMap
      );
   ...
   */
  static decodeSphericalHarmonicsFromSectionHeader(
    fieldNames: string[],
    fieldNameIdMap: { [key: string]: number }
  ): SphericalHarmonicsInfo {
    ...
    let degree1Fields: number[] = [];
    let degree2Fields: number[] = [];
   ...
   */
  static getHeaderSectionNames(headerLines: string[]): string[] {
   ...
   */
  static checkTextForEndHeader(endHeaderTestText: string): boolean {
   ...
   * @returns True if end header token is found
   */
  static checkBufferForEndHeader(
    buffer: ArrayBuffer,
    searchOffset: number,
    chunkSize: number,
    decoder: TextDecoder
    ...
    const endHeaderTestChunk = new Uint8Array(
      buffer,
      Math.max(0, searchOffset - chunkSize),
      chunkSize
    );
   ...
   * @returns Header text
   */
  static extractHeaderFromBufferToText(plyBuffer: ArrayBuffer): string {
   ...
   * @returns Header text
   */
  static readHeaderFromBuffer(plyBuffer: ArrayBuffer): string {
   ...
   */
  static convertHeaderTextToLines(headerText: string): string[] {
   ...
   * @returns PLY format
   */
  static determineHeaderFormatFromHeaderText(headerText: string): PlyFormat {
   ...
   * @returns PLY format
   */
  static determineHeaderFormatFromPlyBuffer(plyBuffer: ArrayBuffer): PlyFormat {
