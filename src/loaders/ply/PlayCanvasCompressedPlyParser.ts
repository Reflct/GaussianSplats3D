 ...
 */
const unpack111011 = (result: THREE.Vector3, value: number): void => {
 ...
 */
const unpack8888 = (result: THREE.Vector4, value: number): void => {
 ...
 */
const unpackRot = (result: THREE.Quaternion, value: number): void => {
...
export class PlayCanvasCompressedPlyParser {
  ...
  static decodeHeaderText(headerText: string): PlayCanvasHeader {
    ...
    let shElement: PlyElement | undefined;
  ...
  static decodeHeader(plyBuffer: ArrayBuffer): PlayCanvasHeader {
     ...
     */
    const find = (buf: Uint8Array, search: Uint8Array): number => {
  ...
  static readElementData(
    element: PlyElement | undefined,
    readBuffer: DataView<ArrayBuffer> | ArrayBuffer,
    readOffset: number,
    fromIndex?: number,
    toIndex?: number,
    propertyFilter: ((name: string) => boolean) | null = null
  ): number {
    if (!element) return readOffset;
   ...
   */
  static getElementStorageArrays(
    chunkElement?: PlyElement,
    vertexElement?: PlyElement,
    shElement?: PlyElement
  ): ElementStorageArrays {
  ...
  static parseToUncompressedSplatBufferSection(
    chunkElement: PlyElement | undefined,
    vertexElement: PlyElement | undefined,
    fromIndex: number,
    toIndex: number,
    chunkSplatIndexOffset: number,
    vertexDataBuffer: DataView<ArrayBuffer>,
    ...
    for (let i = fromIndex; i <= toIndex; ++i) {
      if (!positionExtremes || !scaleExtremes || !colorExtremes) continue;
  ...
  static parseSphericalHarmonicsToUncompressedSplatArraySection(
    chunkElement: PlyElement | undefined,
    shElement: PlyElement | undefined,
    fromIndex: number,
    toIndex: number,
    vertexDataBuffer: DataView<ArrayBuffer>,
    vertexReadOffset: number,
    outSphericalHarmonicsDegree: number,
    readSphericalHarmonicsDegree: number,
    splatArray: UncompressedSplatArray,
    propertyFilter: ((name: string) => boolean) | null = null
  ...
  static parseToUncompressedSplatArray(
    ...
    if (!vertexElement) return splatArray;
