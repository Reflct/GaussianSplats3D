...
const toUint8 = (v: number, rangeMin: number, rangeMax: number): number => {
  v = clamp(v, rangeMin, rangeMax);
  const range = rangeMax - rangeMin;
  return clamp(Math.floor(((v - rangeMin) / range) * 255), 0, 255);
};

const fromUint8 = (v: number, rangeMin: number, rangeMax: number): number => {
  const range = rangeMax - rangeMin;
  return (v / 255) * range + rangeMin;
};

const fromHalfFloatToUint8 = (
  v: number,
  rangeMin: number,
  rangeMax: number
): number => {
...
const convertBetweenCompressionLevels = (function () {
  ...
  return function (
    ...
    let outputConversionFunc: (v: number) => number = noop;
    ...
    if (fromLevel === 2 && isSH) {
      ...
      if (toLevel === 0) outputConversionFunc = fromHalfFloat;
        ...
        else outputConversionFunc = (v) => fromHalfFloatToUint8(v, 0, 255);
      }
    } else if (fromLevel === 0) {
      if (toLevel === 1) outputConversionFunc = toHalfFloat;
      else if (toLevel == 2) {
        if (!isSH) outputConversionFunc = toHalfFloat;
        else outputConversionFunc = (v) => toUint8(v, 0, 255);
      }
    }
...
export interface SplatBufferSection {
  compressionLevel: number;
  ...
  partiallyFilledBucketLengths: number[] | Uint32Array;
  dataOffset: number;
  dataSize: number;
  storedBucketBlockSize: number;
  bufferOffset: number;
  splatCountOffset?: number;
  dataBase?: number;
  compressionScaleFactor?: number;
  compressionScaleRange?: number;
  bucketArray?: Float32Array;
  bucketIndex?: number[];
  bucketsSplatOffset?: number[];
  bucketsSplatCount?: number[];
  bucketsAddress?: number[];
  ...
  splatDataStorageSizeBytes?: number;
...
export class SplatBuffer {
  ...
  static CompressionLevels: { [key: number]: CompressionLevelConfig } = {
    0: {
      BytesPerCenter: 12,
      BytesPerScale: 12,
      BytesPerRotation: 16,
  ...
  sections: SplatBufferSection[] = [];
  splatCount: number = 0;
  maxSplatCount: number = 0;
  globalSplatIndexToSectionMap: number[] = [];
  ...
  constructor(bufferData: ArrayBuffer, secLoadedCountsToMax: boolean = true) {
  ...
  getBucketIndex(section: SplatBufferSection, localSplatIndex: number): number {
    let bucketIndex;
  ...
  getSplatCenter(
    ...
    const dataView = new DataView(
      section.dataArray as ArrayBuffer,
      (section.dataBase || 0) + srcSplatCentersBase
    );
    ...
    if (section.compressionLevel >= 1) {
      ...
      const sf = section.compressionScaleFactor as number;
      const sr = section.compressionScaleRange as number;
      outCenter.x =
        (x - sr) * sf + (section.bucketArray as Float32Array)[bucketBase];
      outCenter.y =
        (y - sr) * sf + (section.bucketArray as Float32Array)[bucketBase + 1];
      outCenter.z =
        (z - sr) * sf + (section.bucketArray as Float32Array)[bucketBase + 2];
    } else {
      outCenter.x = x;
      outCenter.y = y;
      outCenter.z = z;
  ...
  getSplatScaleAndRotation = (function () {
    ...
    return function (
      ...
      transform?: THREE.Matrix4,
      scaleOverride?: { x?: number; y?: number; z?: number }
    ): void {
      ...
      const dataView = new DataView(
        section.dataArray as ArrayBuffer,
        (section.dataBase || 0) + srcSplatScalesBase
      );
  ...
  getSplatColor(
    ...
    const splatColorsArray = new Uint8Array(
      section.dataArray as ArrayBuffer,
      (section.dataBase || 0) + srcSplatColorsBase,
      4
    );
  ...
  fillSplatCenterArray(
    outCenterArray: Float32Array | number[],
    transform?: THREE.Matrix4,
    srcFrom?: number,
    srcTo?: number,
    destFrom?: number
  ...
  fillSplatScaleRotationArray = (function () {
    ...
    const ensurePositiveW = (quaternion: THREE.Quaternion): void => {
    ...
    return function (
      this: SplatBuffer,
      outScaleArray?: Float32Array | number[] | null,
      outRotationArray?: Float32Array | number[] | null,
      transform?: THREE.Matrix4,
      srcFrom?: number,
      srcTo?: number,
      destFrom?: number,
      desiredOutputCompressionLevel?: number,
      scaleOverride?: { x?: number; y?: number; z?: number }
    ): void {
      const splatCount = this.splatCount;
      ...
      const outputConversion = (
        value: number,
        srcCompressionLevel?: number
      ): number => {
        if (srcCompressionLevel === undefined) srcCompressionLevel = 0; // Default to uncompressed
        return convertBetweenCompressionLevels(
          value,
          srcCompressionLevel,
          desiredOutputCompressionLevel || 0
        );
      };
      ...
      for (let i = srcFrom; i <= srcTo; i++) {
        ...
        scale.set(
          toUncompressedFloat(srcScaleX, section.compressionLevel),
          toUncompressedFloat(srcScaleY, section.compressionLevel),
          toUncompressedFloat(srcScaleZ, section.compressionLevel)
        );

        rotation
          .set(
            toUncompressedFloat(srcRotationX, section.compressionLevel),
            toUncompressedFloat(srcRotationY, section.compressionLevel),
            toUncompressedFloat(srcRotationZ, section.compressionLevel),
            toUncompressedFloat(srcRotationW, section.compressionLevel)
          )
          .normalize();
  ...
  static computeCovariance = (function () {
    ...
    return function (
      ...
      transform: THREE.Matrix4 | null | undefined,
      outCovariance: Float32Array | number[],
      outOffset: number = 0,
      desiredOutputCompressionLevel?: number
    ): void {
  ...
  fillSplatCovarianceArray(
    covarianceArray: Float32Array | number[],
    transform?: THREE.Matrix4,
    srcFrom?: number,
    srcTo?: number,
    destFrom?: number,
  ...
  fillSplatColorArray(
    outColorArray: Uint8Array | number[],
    minimumAlpha: number = 0,
    srcFrom?: number,
    srcTo?: number,
    destFrom?: number
  ): void {
  ...
  static calculateComponentStorage(
    compressionLevel: number,
    sphericalHarmonicsDegree: number
  ): {
    bytesPerCenter: number;
    ...
    bytesPerRotation: number;
    bytesPerColor: number;
    sphericalHarmonicsComponentsPerSplat: number;
    sphericalHarmonicsBytesPerSplat: number;
    bytesPerSplat: number;
  } {
  ...
  updateLoadedCounts(newSectionCount: number, newSplatCount: number): void {
  ...
  updateSectionLoadedCounts(sectionIndex: number, newSplatCount: number): void {
  ...
  fillSphericalHarmonicsArray = (function () {
    const sphericalHarmonicVectors: THREE.Vector3[] = [];
    for (let i = 0; i < 15; i++) {
    ...
    const copy3 = (srcArray: number[], destArray: number[]): void => {
    ...
    return function (
      this: SplatBuffer,
      outSphericalHarmonicsArray: Float32Array | number[],
      ...
      const localFromHalfFloatToUint8 = (v: number): number => {
      ...
      const localToUint8 = (v: number): number => {
        return toUint8(
          v,
          this.minSphericalHarmonicsCoeff!,
          this.maxSphericalHarmonicsCoeff!
      ...
      for (let i = srcFrom; i <= srcTo; i++) {
        ...
        let outputConversionFunc: (v: number) => number = noop;
        if (
          compressionLevelForOutputConversion !==
            desiredOutputCompressionLevel &&
  ...
  static parseHeader(buffer: ArrayBuffer): SplatBufferHeader {
  ...
  static writeHeaderCountsToBuffer(
    sectionCount: number,
    splatCount: number,
    buffer: ArrayBuffer
  ): void {
  ...
  static writeHeaderToBuffer(
    header: SplatBufferHeader,
    buffer: ArrayBuffer
  ): void {
    const headerArrayUint8 = new Uint8Array(
      buffer,
  ...
  static parseSectionHeaders(
    ...
    const sectionHeaders: SplatBufferSection[] = [];
    ...
    for (let i = 0; i < maxSectionCount; i++) {
      ...
      const bytesPerSplat = this.calculateComponentStorage(
        compressionLevel,
  ...
  static writeSectionHeaderToBuffer(
    sectionHeader: SplatBufferSection,
    compressionLevel: number,
    buffer: ArrayBuffer,
    offset: number = 0
  ): void {
  ...
  static writeSectionHeaderSplatCountToBuffer(
    splatCount: number,
    buffer: ArrayBuffer,
    offset: number = 0
  ): void {
  ...
  static writeSplatDataToSectionBuffer = (function () {
    ...
    return function (
      ...
      compressionLevel: number,
      sphericalHarmonicsDegree: number,
      bucketCenter: THREE.Vector3 = new THREE.Vector3(0, 0, 0),
      compressionScaleFactor: number = 1.0,
      compressionScaleRange: number = 1024,
      minSphericalHarmonicsCoeff: number = -DefaultSphericalHarmonics8BitCompressionHalfRange,
      maxSphericalHarmonicsCoeff: number = DefaultSphericalHarmonics8BitCompressionHalfRange
    ): void {
      const sphericalHarmonicsComponentsPerSplat =
        getSphericalHarmonicsComponentCountForDegree(sphericalHarmonicsDegree);
      const bytesPerCenter =
  ...
  static generateFromUncompressedSplatArrays(
    ...
    let minSphericalHarmonicsCoeff: number | undefined;
    let maxSphericalHarmonicsCoeff: number | undefined;
    ...
    const sectionBuffers: ArrayBuffer[] = [];
    const sectionHeaderBuffers: ArrayBuffer[] = [];
    ...
    for (let sa = 0; sa < splatArrays.length; sa++) {
      ...
      if (compressionLevel >= 1) {
        const bucketMetaDataArray = new Uint32Array(
          sectionBuffer,
          0,
          partiallyFullBucketLengths.length
        );
      ...
      SplatBuffer.writeSectionHeaderToBuffer(
        {
          ...
          sphericalHarmonicsCount:
            getSphericalHarmonicsComponentCountForDegree(shDegree),
  ...
  static computeBucketsForUncompressedSplatArray(
    ...
    fullBuckets: Array<{ splats: number[]; center: number[] }>;
    partiallyFullBuckets: Array<{ splats: number[]; center: number[] }>;
  } {
    ...
    const fullBuckets: Array<{ splats: number[]; center: number[] }> = [];
    const partiallyFullBuckets: {
      [key: string]: { splats: number[]; center: number[] } | null;
    } = {};
    ...
    const partiallyFullBucketArray: Array<{
      splats: number[];
      center: number[];
    }> = [];
  ...
  static preallocateUncompressed(
    splatCount: number,
    sphericalHarmonicsDegrees: number
  ): {
    splatBuffer: SplatBuffer;
    splatBufferDataOffsetBytes: number;
  } {
    const shDescriptor =
    ...
    const splatBufferSizeBytes =
      splatBufferDataOffsetBytes + shDescriptor.BytesPerSplat * splatCount;
    ...
    SplatBuffer.writeSectionHeaderToBuffer(
      {
        ...
        sphericalHarmonicsCount: getSphericalHarmonicsComponentCountForDegree(
          sphericalHarmonicsDegrees
        ),
        partiallyFilledBucketLengths: [],
        dataOffset: 0,
