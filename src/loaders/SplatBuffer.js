...
exports.SplatBuffer = void 0;
const THREE = __importStar(require("three"));
const UncompressedSplatArray_1 = require("./UncompressedSplatArray");
const Util_1 = require("../Util");
const Constants_1 = require("../Constants");
const DefaultSphericalHarmonics8BitCompressionRange = Constants_1.Constants.SphericalHarmonics8BitCompressionRange;
...
const toUncompressedFloat = (f, compressionLevel, isSH = false, range8BitMin, range8BitMax) => {
    ...
    return f; // Default case to satisfy TypeScript
};
const toUint8 = (v, rangeMin, rangeMax) => {
    v = (0, Util_1.clamp)(v, rangeMin, rangeMax);
    const range = rangeMax - rangeMin;
    return (0, Util_1.clamp)(Math.floor(((v - rangeMin) / range) * 255), 0, 255);
};
...
const dataViewFloatForCompressionLevel = (dataView, floatIndex, compressionLevel, isSH = false) => {
    if (compressionLevel === 0) {
        ...
        return dataView.getUint8(floatIndex);
    }
};
const convertBetweenCompressionLevels = (function () {
    ...
    return function (val, fromLevel, toLevel, isSH = false) {
        ...
        if (fromLevel === 2 && isSH) {
            if (toLevel === 1)
                outputConversionFunc = (v) => fromUint8ToHalfFloat(v, 0, 255);
            else if (toLevel == 0) {
                outputConversionFunc = (v) => fromUint8(v, 0, 255);
            }
        }
            ...
            if (toLevel === 0)
                ...
                if (!isSH)
                    outputConversionFunc = noop;
                else
                    outputConversionFunc = (v) => fromHalfFloatToUint8(v, 0, 255);
            }
        }
        else if (fromLevel === 0) {
            if (toLevel === 1)
                ...
                if (!isSH)
                    ...
                    outputConversionFunc = (v) => toUint8(v, 0, 255);
            }
        }
...
class SplatBuffer {
    constructor(bufferData, secLoadedCountsToMax = true) {
        ...
        this.getSplatScaleAndRotation = (function () {
            ...
            return function (index, outScale, outRotation, transform, scaleOverride) {
                ...
                const localSplatIndex = index - (section.splatCountOffset || 0);
                const srcSplatScalesBase = section.bytesPerSplat * localSplatIndex +
                    SplatBuffer.CompressionLevels[section.compressionLevel]
                        .ScaleOffsetBytes;
                const dataView = new DataView(section.dataArray, (section.dataBase || 0) + srcSplatScalesBase);
                scale.set(toUncompressedFloat(dataViewFloatForCompressionLevel(dataView, 0, section.compressionLevel), section.compressionLevel), toUncompressedFloat(dataViewFloatForCompressionLevel(dataView, 1, section.compressionLevel), section.compressionLevel), toUncompressedFloat(dataViewFloatForCompressionLevel(dataView, 2, section.compressionLevel), section.compressionLevel));
                if (scaleOverride) {
                    if (scaleOverride.x !== undefined)
                        scale.x = scaleOverride.x;
                    if (scaleOverride.y !== undefined)
                        scale.y = scaleOverride.y;
                    if (scaleOverride.z !== undefined)
                        scale.z = scaleOverride.z;
                }
                rotation.set(toUncompressedFloat(dataViewFloatForCompressionLevel(dataView, 4, section.compressionLevel), section.compressionLevel), toUncompressedFloat(dataViewFloatForCompressionLevel(dataView, 5, section.compressionLevel), section.compressionLevel), toUncompressedFloat(dataViewFloatForCompressionLevel(dataView, 6, section.compressionLevel), section.compressionLevel), toUncompressedFloat(dataViewFloatForCompressionLevel(dataView, 3, section.compressionLevel), section.compressionLevel));
                if (transform) {
        ...
        this.fillSplatScaleRotationArray = (function () {
            const scaleMatrix = new THREE.Matrix4();
            const rotationMatrix = new THREE.Matrix4();
            const tempMatrix = new THREE.Matrix4();
            const scale = new THREE.Vector3();
            ...
            return function (outScaleArray, outRotationArray, transform, srcFrom, srcTo, destFrom, desiredOutputCompressionLevel, scaleOverride) {
                ...
                if (destFrom === undefined)
                    destFrom = srcFrom;
                const outputConversion = (value, srcCompressionLevel) => {
                    if (srcCompressionLevel === undefined)
                        srcCompressionLevel = 0; // Default to uncompressed
                    return convertBetweenCompressionLevels(value, srcCompressionLevel, desiredOutputCompressionLevel || 0);
                };
                for (let i = srcFrom; i <= srcTo; i++) {
                    ...
                    const dataView = new DataView(section.dataArray, (section.dataBase || 0) + srcSplatScalesBase);
                    const srcScaleX = scaleOverride && scaleOverride.x !== undefined
                        ? scaleOverride.x
                        : dataViewFloatForCompressionLevel(dataView, 0, section.compressionLevel);
                    const srcScaleY = scaleOverride && scaleOverride.y !== undefined
                        ? scaleOverride.y
                        : dataViewFloatForCompressionLevel(dataView, 1, section.compressionLevel);
                    const srcScaleZ = scaleOverride && scaleOverride.z !== undefined
                        ? scaleOverride.z
                        : dataViewFloatForCompressionLevel(dataView, 2, section.compressionLevel);
                    const srcRotationW = dataViewFloatForCompressionLevel(dataView, 3, section.compressionLevel);
                    const srcRotationX = dataViewFloatForCompressionLevel(dataView, 4, section.compressionLevel);
                    const srcRotationY = dataViewFloatForCompressionLevel(dataView, 5, section.compressionLevel);
                    const srcRotationZ = dataViewFloatForCompressionLevel(dataView, 6, section.compressionLevel);
                    scale.set(toUncompressedFloat(srcScaleX, section.compressionLevel), toUncompressedFloat(srcScaleY, section.compressionLevel), toUncompressedFloat(srcScaleZ, section.compressionLevel));
                    rotation
                        .set(toUncompressedFloat(srcRotationX, section.compressionLevel), toUncompressedFloat(srcRotationY, section.compressionLevel), toUncompressedFloat(srcRotationZ, section.compressionLevel), toUncompressedFloat(srcRotationW, section.compressionLevel))
                        .normalize();
                    if (transform) {
        ...
        this.fillSphericalHarmonicsArray = (function () {
            ...
            return function (outSphericalHarmonicsArray, outSphericalHarmonicsDegree, transform, srcFrom, srcTo, destFrom, desiredOutputCompressionLevel) {
                ...
                for (let i = srcFrom; i <= srcTo; i++) {
                    ...
                    const outSphericalHarmonicsComponentsCount = (0, Util_1.getSphericalHarmonicsComponentCountForDegree)(outSphericalHarmonicsDegree);
                    const localSplatIndex = this.globalSplatIndexToLocalSplatIndexMap[i];
                    ...
                    let compressionLevelForOutputConversion = transform
                        ? 0
                        : section.compressionLevel;
                    let outputConversionFunc = noop;
                    if (compressionLevelForOutputConversion !==
                        desiredOutputCompressionLevel &&
                        desiredOutputCompressionLevel !== undefined) {
                    ...
                    if (outSphericalHarmonicsDegree >= 1) {
                        set3FromArray(shIn1, dataView, 3, 0, section.compressionLevel);
                        set3FromArray(shIn2, dataView, 3, 1, section.compressionLevel);
                        set3FromArray(shIn3, dataView, 3, 2, section.compressionLevel);
                        if (transform) {
                            toUncompressedFloatArray3(shIn1, shIn1, section.compressionLevel, minShCoeff, maxShCoeff);
                            toUncompressedFloatArray3(shIn2, shIn2, section.compressionLevel, minShCoeff, maxShCoeff);
                            toUncompressedFloatArray3(shIn3, shIn3, section.compressionLevel, minShCoeff, maxShCoeff);
                        ...
                        if (outSphericalHarmonicsDegree >= 2) {
                            set3FromArray(shIn1, dataView, 3, 9, section.compressionLevel);
                            set3FromArray(shIn2, dataView, 3, 10, section.compressionLevel);
                            set3FromArray(shIn3, dataView, 3, 11, section.compressionLevel);
                            set3FromArray(shIn4, dataView, 3, 12, section.compressionLevel);
                            set3FromArray(shIn5, dataView, 3, 13, section.compressionLevel);
                            if (transform) {
                                toUncompressedFloatArray3(shIn1, shIn1, section.compressionLevel, minShCoeff, maxShCoeff);
                                toUncompressedFloatArray3(shIn2, shIn2, section.compressionLevel, minShCoeff, maxShCoeff);
                                toUncompressedFloatArray3(shIn3, shIn3, section.compressionLevel, minShCoeff, maxShCoeff);
                                toUncompressedFloatArray3(shIn4, shIn4, section.compressionLevel, minShCoeff, maxShCoeff);
                                toUncompressedFloatArray3(shIn5, shIn5, section.compressionLevel, minShCoeff, maxShCoeff);
    ...
    getSplatCenter(globalSplatIndex, outCenter, transform) {
        ...
        const section = this.sections[sectionIndex];
        const localSplatIndex = globalSplatIndex - (section.splatCountOffset || 0);
        const srcSplatCentersBase = section.bytesPerSplat * localSplatIndex;
        const dataView = new DataView(section.dataArray, (section.dataBase || 0) + srcSplatCentersBase);
        const x = dataViewFloatForCompressionLevel(dataView, 0, section.compressionLevel);
        const y = dataViewFloatForCompressionLevel(dataView, 1, section.compressionLevel);
        const z = dataViewFloatForCompressionLevel(dataView, 2, section.compressionLevel);
        if (section.compressionLevel >= 1) {
            ...
            outCenter.x =
                (x - sr) * sf + section.bucketArray[bucketBase];
            outCenter.y =
                (y - sr) * sf + section.bucketArray[bucketBase + 1];
            outCenter.z =
                (z - sr) * sf + section.bucketArray[bucketBase + 2];
        }
        else {
            outCenter.x = x;
            outCenter.y = y;
        ...
        if (transform)
            outCenter.applyMatrix4(transform);
    }
    getSplatColor(globalSplatIndex, outColor) {
        ...
        const srcSplatColorsBase = section.bytesPerSplat * localSplatIndex +
            SplatBuffer.CompressionLevels[section.compressionLevel].ColorOffsetBytes;
        const splatColorsArray = new Uint8Array(section.dataArray, (section.dataBase || 0) + srcSplatColorsBase, 4);
    ...
    fillSplatColorArray(outColorArray, minimumAlpha = 0, srcFrom, srcTo, destFrom) {
        const splatCount = this.splatCount;
        srcFrom = srcFrom || 0;
        srcTo = srcTo || splatCount - 1;
        if (destFrom === undefined)
        ...
        for (let i = srcFrom; i <= srcTo; i++) {
            ...
            const srcSplatColorsBase = section.bytesPerSplat * localSplatIndex +
                SplatBuffer.CompressionLevels[section.compressionLevel]
                    .ColorOffsetBytes;
            const dataView = new Uint8Array(section.dataArray, (section.dataBase || 0) + srcSplatColorsBase, 4);
    ...
    constructFromBuffer(bufferData, secLoadedCountsToMax) {
        ...
        this.maxSectionCount = header.maxSectionCount || 0;
        this.sectionCount = secLoadedCountsToMax ? header.maxSectionCount || 0 : 0;
        this.maxSplatCount = header.maxSplatCount;
        this.splatCount = secLoadedCountsToMax ? header.maxSplatCount : 0;
        this.compressionLevel = header.compressionLevel || 0;
        if (header.sceneCenter) {
            this.sceneCenter.copy(header.sceneCenter);
        }
    ...
    static calculateComponentStorage(compressionLevel, sphericalHarmonicsDegree) {
        ...
        const sphericalHarmonicsComponentsPerSplat = (0, Util_1.getSphericalHarmonicsComponentCountForDegree)(sphericalHarmonicsDegree);
        const sphericalHarmonicsBytesPerSplat = SplatBuffer.CompressionLevels[compressionLevel]
    ...
    linkBufferArrays() {
        for (let i = 0; i < (this.maxSectionCount || 0); i++) {
            const section = this.sections[i];
            if (!section)
                continue;
            section.bucketArray = new Float32Array(this.bufferData, section.bucketsBase || 0, section.bucketCount * SplatBuffer.BucketStorageSizeFloats);
            if ((section.partiallyFilledBucketCount || 0) > 0) {
                section.partiallyFilledBucketLengths = new Uint32Array(this.bufferData, section.base || 0, section.partiallyFilledBucketCount);
            }
        }
    }
    buildMaps() {
    ...
    updateSectionLoadedCounts(sectionIndex, newSplatCount) {
        const sectionHeaderOffset = SplatBuffer.HeaderSizeBytes +
    ...
    static parseHeader(buffer) {
        ...
        const minSphericalHarmonicsCoeff = headerArrayFloat32[9] ||
            -DefaultSphericalHarmonics8BitCompressionHalfRange;
        const maxSphericalHarmonicsCoeff = headerArrayFloat32[10] ||
            DefaultSphericalHarmonics8BitCompressionHalfRange;
        return {
            versionMajor,
            versionMinor,
    ...
    static parseSectionHeaders(header, buffer, offset = 0, secLoadedCountsToMax) {
        ...
        const maxSectionCount = header.maxSectionCount || 0;
        ...
        let sectionBase = SplatBuffer.HeaderSizeBytes +
            maxSectionCount * SplatBuffer.SectionHeaderSizeBytes;
        ...
        for (let i = 0; i < maxSectionCount; i++) {
            ...
            const bytesPerSplat = this.calculateComponentStorage(compressionLevel, sphericalHarmonicsDegree).bytesPerSplat;
            ...
            const sectionHeader = {
                ...
                storedBucketBlockSize: bucketBlockSize,
                bucketsAllocated: 0,
                fullBucketCount: fullBucketCount,
                validBucketCount: partiallyFilledBucketCount,
                bytesPerSplat: bytesPerSplat,
                sphericalHarmonicsCount: 0, // Will be calculated later
                partiallyFilledBucketLengths: [],
                dataOffset: sectionBase + bucketsStorageSizeBytes,
                dataSize: storageSizeBytes,
                bufferOffset: sectionBase,
    ...
    static writeSectionHeaderToBuffer(sectionHeader, compressionLevel, buffer, offset = 0) {
        const sectionHeaderArrayUint16 = new Uint16Array(buffer, offset, SplatBuffer.SectionHeaderSizeBytes / 2);
        const sectionHeaderArrayUint32 = new Uint32Array(buffer, offset, SplatBuffer.SectionHeaderSizeBytes / 4);
        const sectionHeaderArrayFloat32 = new Float32Array(buffer, offset, SplatBuffer.SectionHeaderSizeBytes / 4);
        sectionHeaderArrayUint32[0] = sectionHeader.splatCount;
        sectionHeaderArrayUint32[1] = sectionHeader.maxSplatCount;
        sectionHeaderArrayUint32[2] =
            compressionLevel >= 1 ? sectionHeader.bucketSize : 0;
        sectionHeaderArrayUint32[3] =
            compressionLevel >= 1 ? sectionHeader.bucketCount : 0;
        sectionHeaderArrayFloat32[4] =
            compressionLevel >= 1 ? sectionHeader.storedBucketBlockSize || 0.0 : 0.0;
        sectionHeaderArrayUint16[10] =
            compressionLevel >= 1 ? SplatBuffer.BucketStorageSizeBytes : 0;
        sectionHeaderArrayUint32[6] =
            compressionLevel >= 1 ? sectionHeader.compressionScaleRange || 0 : 0;
        sectionHeaderArrayUint32[7] = sectionHeader.dataSize;
        sectionHeaderArrayUint32[8] =
            compressionLevel >= 1 ? sectionHeader.fullBucketCount : 0;
        sectionHeaderArrayUint32[9] =
            compressionLevel >= 1 ? sectionHeader.validBucketCount : 0;
    ...
    static writeSectionHeaderSplatCountToBuffer(splatCount, buffer, offset = 0) {
        const sectionHeaderArrayUint32 = new Uint32Array(buffer, offset, SplatBuffer.SectionHeaderSizeBytes / 4);
        sectionHeaderArrayUint32[0] = splatCount;
    }
    static generateFromUncompressedSplatArrays(splatArrays, minimumAlpha, compressionLevel, sceneCenter, blockSize, bucketSize, options = []) {
        ...
        for (let sa = 0; sa < splatArrays.length; sa++) {
            ...
            for (let i = 0; i < splatArray.splats.length; i++) {
                ...
                for (let sc = UncompressedSplatArray_1.UncompressedSplatArray.OFFSET.FRC0; sc < UncompressedSplatArray_1.UncompressedSplatArray.OFFSET.FRC23 && sc < splat.length; sc++) {
                    if (minSphericalHarmonicsCoeff === undefined ||
                        splat[sc] < minSphericalHarmonicsCoeff) {
                        minSphericalHarmonicsCoeff = splat[sc];
                    }
                    if (maxSphericalHarmonicsCoeff === undefined ||
        ...
        for (let sa = 0; sa < splatArrays.length; sa++) {
            const splatArray = splatArrays[sa];
            const validSplats = new UncompressedSplatArray_1.UncompressedSplatArray(shDegree);
            for (let i = 0; i < splatArray.splatCount; i++) {
                const targetSplat = splatArray.splats[i];
                if ((targetSplat[UncompressedSplatArray_1.UncompressedSplatArray.OFFSET.OPACITY] || 0) >=
                    minimumAlpha) {
                    validSplats.addSplat(targetSplat);
                }
            }
            const sectionOptions = options[sa] || {};
            const sectionBlockSize = (sectionOptions.blockSizeFactor || 1) *
                (blockSize || SplatBuffer.BucketBlockSize);
            const sectionBucketSize = Math.ceil((sectionOptions.bucketSizeFactor || 1) *
            ...
            const bucketDataBytes = compressionLevel >= 1
            ...
            if (compressionLevel >= 1) {
                const bucketMetaDataArray = new Uint32Array(sectionBuffer, 0, partiallyFullBucketLengths.length);
            ...
            SplatBuffer.writeSectionHeaderToBuffer({
                ...
                storedBucketBlockSize: sectionBlockSize,
                compressionScaleRange: compressionScaleRange,
                storageSizeBytes: sectionSizeBytes,
                fullBucketCount: fullBucketCount,
                bucketsAllocated: buckets.length,
                validBucketCount: partiallyFilledBucketCount,
                partiallyFilledBucketCount: partiallyFilledBucketCount,
                sphericalHarmonicsDegree: shDegree,
                bytesPerSplat: bytesPerSplat,
                sphericalHarmonicsCount: (0, Util_1.getSphericalHarmonicsComponentCountForDegree)(shDegree),
                partiallyFilledBucketLengths: partiallyFullBucketLengths,
                dataOffset: 0,
                dataSize: sectionSizeBytes,
                bufferOffset: 0,
    ...
    static computeBucketsForUncompressedSplatArray(splatArray, blockSize, bucketSize) {
        ...
        for (let i = 0; i < splatCount; i++) {
            const targetSplat = splatArray.splats[i];
            const center = [
                targetSplat[UncompressedSplatArray_1.UncompressedSplatArray.OFFSET.X],
                targetSplat[UncompressedSplatArray_1.UncompressedSplatArray.OFFSET.Y],
                targetSplat[UncompressedSplatArray_1.UncompressedSplatArray.OFFSET.Z],
            ];
            if (i === 0 || center[0] < min.x)
                min.x = center[0];
            if (i === 0 || center[0] > max.x)
                max.x = center[0];
            if (i === 0 || center[1] < min.y)
                min.y = center[1];
            if (i === 0 || center[1] > max.y)
                max.y = center[1];
            if (i === 0 || center[2] < min.z)
                min.z = center[2];
            if (i === 0 || center[2] > max.z)
                max.z = center[2];
        }
    ...
    static preallocateUncompressed(splatCount, sphericalHarmonicsDegrees) {
        ...
        SplatBuffer.writeSectionHeaderToBuffer({
            ...
            bytesPerSplat: shDescriptor.BytesPerSplat,
            sphericalHarmonicsCount: (0, Util_1.getSphericalHarmonicsComponentCountForDegree)(sphericalHarmonicsDegrees),
            partiallyFilledBucketLengths: [],
            dataOffset: 0,
            dataSize: 0,
            bufferOffset: 0,
...
SplatBuffer.CurrentMajorVersion = 0;
SplatBuffer.CurrentMinorVersion = 1;
SplatBuffer.CenterComponentCount = 3;
SplatBuffer.ScaleComponentCount = 3;
SplatBuffer.RotationComponentCount = 4;
SplatBuffer.ColorComponentCount = 4;
SplatBuffer.CovarianceComponentCount = 6;
SplatBuffer.SplatScaleOffsetFloat = 3;
SplatBuffer.SplatRotationOffsetFloat = 6;
...
SplatBuffer.CovarianceSizeFloats = 6;
SplatBuffer.HeaderSizeBytes = 4096;
SplatBuffer.SectionHeaderSizeBytes = 1024;
SplatBuffer.BucketStorageSizeBytes = 12;
SplatBuffer.BucketStorageSizeFloats = 3;
SplatBuffer.BucketBlockSize = 5.0;
SplatBuffer.BucketSize = 256;
SplatBuffer.computeCovariance = (function () {
    const tempMatrix4 = new THREE.Matrix4();
    ...
    return function (scale, rotation, transform, outCovariance, outOffset = 0, desiredOutputCompressionLevel) {
        ...
        if (desiredOutputCompressionLevel && desiredOutputCompressionLevel >= 1) {
...
SplatBuffer.dot3 = (v1, v2, v3, transformRow, outArray) => {
...
SplatBuffer.addInto3 = (val1, val2, val3, destArray) => {
    destArray[0] = destArray[0] + val1;
    destArray[1] = destArray[1] + val2;
    destArray[2] = destArray[2] + val3;
};
SplatBuffer.dot5 = (v1, v2, v3, v4, v5, transformRow, outArray) => {
    outArray[0] = outArray[1] = outArray[2] = 0;
    const t0 = transformRow[0];
    const t1 = transformRow[1];
    const t2 = transformRow[2];
...
SplatBuffer.rotateSphericalHarmonics3 = (in1, in2, in3, tsh11, tsh12, tsh13, out1, out2, out3) => {
    SplatBuffer.dot3(in1, in2, in3, tsh11, out1);
    SplatBuffer.dot3(in1, in2, in3, tsh12, out2);
    SplatBuffer.dot3(in1, in2, in3, tsh13, out3);
};
SplatBuffer.rotateSphericalHarmonics5 = (in1, in2, in3, in4, in5, tsh11, tsh12, tsh13, tsh21, tsh22, tsh23, tsh24, tsh25, out1, out2, out3, out4, out5) => {
...
SplatBuffer.writeSplatDataToSectionBuffer = (function () {
    ...
    const { X: OFFSET_X, Y: OFFSET_Y, Z: OFFSET_Z, SCALE0: OFFSET_SCALE0, SCALE1: OFFSET_SCALE1, SCALE2: OFFSET_SCALE2, ROTATION0: OFFSET_ROT0, ROTATION1: OFFSET_ROT1, ROTATION2: OFFSET_ROT2, ROTATION3: OFFSET_ROT3, FDC0: OFFSET_FDC0, FDC1: OFFSET_FDC1, FDC2: OFFSET_FDC2, OPACITY: OFFSET_OPACITY, FRC0: OFFSET_FRC0, FRC9: OFFSET_FRC9, } = UncompressedSplatArray_1.UncompressedSplatArray.OFFSET;
    const compressPositionOffset = (v, compressionScaleFactor, compressionScaleRange) => {
        ...
        return (0, Util_1.clamp)(v, 0, doubleCompressionScaleRange);
    };
    return function (targetSplat, sectionBuffer, bufferOffset, compressionLevel, sphericalHarmonicsDegree, bucketCenter = new THREE.Vector3(0, 0, 0), compressionScaleFactor = 1.0, compressionScaleRange = 1024, minSphericalHarmonicsCoeff = -DefaultSphericalHarmonics8BitCompressionHalfRange, maxSphericalHarmonicsCoeff = DefaultSphericalHarmonics8BitCompressionHalfRange) {
        const sphericalHarmonicsComponentsPerSplat = (0, Util_1.getSphericalHarmonicsComponentCountForDegree)(sphericalHarmonicsDegree);
        const bytesPerCenter = SplatBuffer.CompressionLevels[compressionLevel].BytesPerCenter;
        const bytesPerScale = SplatBuffer.CompressionLevels[compressionLevel].BytesPerScale;
        const bytesPerRotation = SplatBuffer.CompressionLevels[compressionLevel].BytesPerRotation;
        ...
        if (targetSplat[OFFSET_SCALE0] !== undefined) {
            tempScale.set(targetSplat[OFFSET_SCALE0] || 0, targetSplat[OFFSET_SCALE1] || 0, targetSplat[OFFSET_SCALE2] || 0);
        }
        else {
            tempScale.set(0, 0, 0);
        }
